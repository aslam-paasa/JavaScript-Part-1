...
/** 
 * => Now it is the responsibility of the createOrder api to create an order first 
 *    of all and then will call our callback function back once the order is created
 *    with the orderId.
 * => This is how we are using async operations using callback but there is very 
 *    important issue with this code - "Inversion Of Control". 
 * 
 ...
 *    }
 * 
 * => And now we are just sitting back as a developer that at some point of time this
 *    createOrder function will call our callback function back and everything will be
 *    peaceful.
 * => But what if we are just blindly trusting this createOrder api and it never calls
 *    our callback function back or it might call it twice. Because this createOrder api
 *    can be in some other server or external world or written be some other developer 
 *    or intern so we can' blindly trust these api.
 * => So, we meant to say that we have given the responsibility of our code 
 *    "proceedToPayment(orderId);" to some other api and it's not in our control anymore, 
 *    we have just passed in and expecting it to randomly/magically being called after 
 *    the order is created. Isn't it risky?
 * => So passing callback functions like this isn't reliable, we are just giving the 
 *    control of our program to some other part of code which we are not aware of. 
 *    This is not a very confident way of writing code.
*/

/**
 * Q. How we can solve this Inversion Of Control?
 * => We will see how we can handle these types of situations using promises.
 * => If we would designing these api's, we would have designed our createOrder() API in such
 *    a way that createOrder() API will no longer take a callback function but it will just 
 *    take card details and it will return us a promise.
 * 
 *    const promise = createOrder(cart);
 * 
 * => And we will capture that promise in a veriable 'promise. So, whenever we call this line, 
 *    this createOrder(cart) api will return us a promise and promise is nothing but we can 
 *    assume an empty object with some data value in it:
 *    {data: value}, 
 *    and this data value will hold whatever this createOrder api will return to us.
 * 
 * => This createOrder API is an async operation, it will take some time to execute but we don't
 *    know how much time it will take but as soon as this line is executed:
 *    const promise = createOrder(cart);
 *    it will return us an object with a data having some undefined property:
 *    {data: undefined} -> undefined means empty value
 * 
 * => And suppose after 5-6 seconds, whenever how much ever time it takes, it 
 *    will create an order and will give us order detail and will fill these 
 *    objects back at some later point of time.
 *    {data: orderDetails}
*/

/**
 * Q. What will happen once we have there orderDetails/data with us?
 * Q. How do we continue our program?
 * Q. What will happen to proceedToPayment?
 * => Now we will attach callback function - "promise" to this promise object 
 *    and for doing that we will use ".then()"
 * => This is the function which is available over promise object, so we will
 *    use promise.then() and we will attach a callback function using: .then()
 * 
 *    promise.then(function(orderId) {
 *       proceedToPayment(orderId);
 *    });
 * 
 * => Earlier this {data: undefined} used to be undefined, but as soon as JS executed
 *    this line "const promise = createOrder(cart). This was empty.
 * => But after certain point of time, this will be filled with exact data that will 
 *    be returned by createOrder api - {data: orderDetails}
 * => And once we have data inside this promise object - {data: orderDetails}, the 
 *    callback function "proceedToPayment" that we attacked to this promise object 
