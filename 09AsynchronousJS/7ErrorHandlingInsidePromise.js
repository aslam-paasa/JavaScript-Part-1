...
// Now as a consumer we have just written the success part, if the orderId is correct then we will do this
// promise.then(function (orderId) {
//     console.log(orderId);
// })
// But what if it fails? how do we write that code?
// For that promise also comes with one more function known as ".catch()", it is just like ".then()" and we can attach a failure callback also to it.
// So the promise object can be attached with a failure callback as well.
// Failure callback means if our callback fails then this callback will be invoked
...
//     console.log(err.message);
//  });
// Now we don't get a red in our console, this is gracefully handling th rejection of a promise. 
// We should also do that whenever we are creating a promise or consuming any promise, we should always write a ".catch()" statement to handle the "error" gracefully.
