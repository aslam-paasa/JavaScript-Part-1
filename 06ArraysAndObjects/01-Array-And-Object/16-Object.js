/* Add two numbers without using "+" operator : (Interview Question)
(a) For both positive numbers or either one negative -
    num1 = 5
    num2 = 2
...
(a) In case of one negative and one positive, then we will check for
       min and max value
    
Q) How we can check for min and max?
 - Through ->
        min = Math.min(num1, num2) = -2
        max = Math.max(num1, num2) =  5
...
let num1 = 22;
let num2 = 2;
if(num1 < 0 && num2 < 0)  // if both nums are negative
{
    for(let i = 1; i < Math.abs(num2); i++) // math.abs return positive value
    {
        num--; // decrement because both are negative value
    } 
    console.log(num1);
}
else
